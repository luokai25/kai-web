...
if(vkeyEl){
  ...
  vkeyEl.value=saved;
  // Update the module-level constant live
  if(saved)window._kaiVideoKey=saved;
  vkeyEl.addEventListener('change',()=>{
    const k=vkeyEl.value.trim();
    localStorage.setItem('kai_video_key',k);
    window._kaiVideoKey=k;
  });
}
